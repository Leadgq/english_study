<template>
  <div class="w-[1200px] mx-auto mt-10 bg-linear-to-br from-blue-50 to-indigo-50 rounded-[20px] p-20 shadow-lg" v-loading="loading">
    <div class="h-20">
      ...
      <div
        ...
        <div class="">
          ...
          <div class="text-sm text-gray-500 mb-1 flex items-center gap-2">{{ item.phonetic }} <el-icon size="18"
              color="#2563EB" @click="playAudio(item)">
              <VideoPlay v-if="!item.isPlaying" />
              <VideoPause v-else />
            </el-icon></div>
    ...
    <div class="mt-10">
      <el-pagination background v-model:current-page="query.page" v-model:page-size="query.pageSize" :total="total"
...
import { Reading, VideoPlay, VideoPause } from '@element-plus/icons-vue'
import { useAudio } from '@/hooks/useAudio';
import type { WordWithIsPlaying } from './type';
...
const getList = async () => {
  try {
    loading.value = true;
    const result = await getWordBookList(query.value);
    if (result.success) {
      total.value = result.data.total;
      list.value = result.data.list.map(item => {
        return {
          ...item,
          isPlaying: false,
        }
